  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute -top-32 -left-24 w-[28rem] h-[28rem] rounded-full opacity-30 blur-3xl"
             style={{ background: 'radial-gradient(circle, hsl(var(--primary) / 0.55), transparent 60%)' }} />
        <div className="absolute top-40 -right-24 w-[26rem] h-[26rem] rounded-full opacity-25 blur-3xl"
             style={{ background: 'radial-gradient(circle, hsl(280 72% 56% / 0.5), transparent 60%)' }} />
      </div>
      ...
      <main className="container py-6 max-w-2xl space-y-6">
        <div className="flex items-center gap-3 animate-fade-in">
          ...
          <div>
            <h2 className="text-2xl font-bold font-display gradient-text">Profile & Settings</h2>
        ...
        <Card className="premium-card animate-slide-up">
          <CardHeader className="pb-4 relative">
            <div className="flex items-center gap-4">
              <label className="relative cursor-pointer group/avatar premium-glow rounded-full">
                <Avatar className="w-20 h-20 ring-2 ring-background shadow-xl">
                  {avatarUrl && <AvatarImage src={avatarUrl} alt={displayName} />}
                  <AvatarFallback className="text-2xl font-bold bg-primary/10 text-primary font-display">{initials}</AvatarFallback>
                </Avatar>
              ...
              <div className="flex-1 min-w-0">
                <CardTitle className="font-display text-xl truncate">{displayName || 'Student'}</CardTitle>
                <CardDescription className="flex items-center gap-2 flex-wrap">
                  <GraduationCap className="w-3.5 h-3.5" /> {studentClass || 'Not set'}
                  {school && <><span>•</span><School className="w-3.5 h-3.5" /> <span className="truncate">{school}</span></>}
                </CardDescription>
              </div>
            </div>
          </CardHeader>
        ...
        <div className="grid grid-cols-3 gap-3 animate-slide-up stagger-1">
          <div className="stat-3d">
            <div className="p-4 text-center relative">
              ...
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Exams</p>
            </div>
          </div>
          <div className="stat-3d">
            <div className="p-4 text-center relative">
        ...
        <Card className="premium-card animate-slide-up stagger-2">
          ...
          <CardContent className="space-y-4 relative">
            ...
            <Button onClick={handleSave} disabled={saving} className="premium-button w-full rounded-xl h-11 gap-2">
